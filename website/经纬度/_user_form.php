...
<form class="machine-account-form" method="post" action="<?= h($action) ?>">
    ...
    <label><span><?= $isEdit ? '新密码' : '密码' ?></span><input type="password" name="password" <?= $isEdit ? '' : 'required' ?> minlength="12" maxlength="128" autocomplete="new-password"><small><?= $isEdit ? '留空则保持原密码' : '至少 12 位' ?></small></label>
    <label><span>确认密码</span><input type="password" name="password_confirm" <?= $isEdit ? '' : 'required' ?> minlength="12" maxlength="128" autocomplete="new-password"></label>
