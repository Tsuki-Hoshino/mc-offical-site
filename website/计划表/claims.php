...
<?php if ($claims): ?>
<section class="plan-claims-toolbar"><div class="plans-tabs" data-claim-tabs><button class="active" data-status="all">全部</button><button data-status="pending">待收集</button><button data-status="completed">已完成</button></div><div class="plan-claims-batch"><button type="button" data-select-visible>全选</button><button type="button" data-clear-selected disabled>清空选择</button><button type="button" class="plans-primary" data-complete-selected disabled>完成所选</button><button type="button" data-uncomplete-selected disabled>撤销完成</button><button type="button" class="danger" data-cancel-selected disabled>取消认领</button><span data-selected-count>当前筛选 0 条 · 未选择</span></div><label class="plans-search"><span>搜索任务</span><input type="search" data-claim-search placeholder="项目、投影或材料"></label></section>
