...
foreach($project['litematics'] as$l){$ltotal=0;$lclaimed=0;$lcollected=0;foreach($l['materials'] as$m){$key=$m['block_id'];$materialIds[$key]=true;if(!isset($materialTotals[$key]))$materialTotals[$key]=['id'=>$key,'name'=>$m['display_name'],'count'=>0,'boxes'=>0,'stacks'=>0,'claimed'=>0,'collected'=>0];$materialTotals[$key]['count']+=(int)$m['block_count'];$materialTotals[$key]['boxes']+=(float)$m['boxes'];$materialTotals[$key]['stacks']+=(float)$m['stacks'];$ltotal+=(float)$m['boxes'];}foreach($l['claims'] as$c){$allClaims[]=$c;$lclaimed+=(float)$c['boxes'];if($c['collected_at'])$lcollected+=(float)$c['boxes'];$mid=(int)$c['material_id'];foreach($l['materials'] as$m)if((int)$m['id']===$mid){$materialTotals[$m['block_id']]['claimed']+=(float)$c['boxes'];if($c['collected_at'])$materialTotals[$m['block_id']]['collected']+=(float)$c['boxes'];break;}}$litematicStats[$l['id']]=['total'=>$ltotal,'claimed'=>$lclaimed,'collected'=>$lcollected,'claim_progress'=>$ltotal>0?min(100,(int)round($lclaimed/$ltotal*100)):0,'collect_progress'=>$lclaimed>0?min(100,(int)round($lcollected/$lclaimed*100)):0];}
...
<div class="material-tools"><?php if($canClaim):?><div class="plan-batch-actions"><button type="button" data-select-page>全选</button><button type="button" data-clear-selection>清空选择</button><button type="button" data-batch-claim disabled>认领所选</button><button type="button" class="primary" data-batch-claim-complete disabled>认领并完成</button><button type="button" data-batch-complete disabled>完成所选</button><button type="button" data-batch-uncomplete disabled>撤销完成</button><button type="button" class="danger" data-batch-delete-claims disabled>取消认领</button><span data-batch-summary>当前筛选 0 项 · 未选择</span></div><?php else:?><div class="material-legend"><span>需求</span><span>已认领</span><span>已收集</span></div><?php endif;?></div>
